...
use super::{
    mmio, VirtioBlkConfig, VIRTIO_MAGIC, VIRTIO_VERSION_LEGACY, VIRTIO_VERSION_MODERN,
};
...
impl VirtioTransport {
    ...
    pub unsafe fn status(&self) -> u32 {
        match self {
            ...
            VirtioTransport::Pci(t) => {
                read_volatile(&(*t.common_cfg).device_status) as u32
            }
        }
    }
    ...
    ///
    /// # Safety
    /// Caller must ensure the transport is properly initialized and
    /// the device is a block device.
    pub unsafe fn read_blk_config(&self) -> VirtioBlkConfig {
        let cfg = match self {
        ...
        VirtioBlkConfig {
            capacity: read_volatile(cfg as *const u64),
            size_max: read_volatile(cfg.add(8) as *const u32),
            seg_max: read_volatile(cfg.add(12) as *const u32),
            geometry_cylinders: read_volatile(cfg.add(16) as *const u16),
            geometry_heads: read_volatile(cfg.add(18) as *const u8),
            geometry_sectors: read_volatile(cfg.add(19) as *const u8),
            blk_size: read_volatile(cfg.add(20) as *const u32),
        }
    }
}
...
pub struct VirtioPciAddrs {
    ...
    /// For modern virtio-pci (0x1040+), this is in subsystem ID (offset 0x2E)
